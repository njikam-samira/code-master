...
import org.uoc.kison.UMGA.utils.Binom;
 ...
 * @author jcasasr
 */
public class GraphReconstructionByNCLog extends GraphReconstruction {

    public static final String PARAM = "NCL";
    public static final String NAME = "Neighbourhood Centrality with Log(n)";
    public static final int minCandidates = 20;

    public GraphReconstructionByNCLog(SimpleIntGraph g, int[] dk) {
        super(g, dk);

        logger.info(String.format("GraphReconstructionByNCL: Edge selection method: '%s'", NAME));
    }
    ...
    protected boolean oneEdgeDeletion() {
        ...
        // randomize the list
        Collections.shuffle(nodesToDeleteEdge_withoutDuplicates, new Random(System.nanoTime()));
        ...
        int maxCandidates = minCandidates + (int) Math.log(Binom.combinations(nodesToDeleteEdge_withoutDuplicates.size() * Math.round(gk.getAverageDegree()) * 2, 2));
        int numCandidates = 0;
        
        // Iterate on 'vi'
        int i = 0;
        while (i < nodesToDeleteEdge_withoutDuplicates.size() && numCandidates <= maxCandidates) {
            ...
            // Iterate on 'vj'
            int j = i + 1;
            while (j < nodesToDeleteEdge_withoutDuplicates.size() && numCandidates <= maxCandidates) {
    ...
    protected boolean oneEdgeAddition() {
        ...
        // randomize the list
        Collections.shuffle(nodesToAddEdge_withoutDuplicates, new Random(System.nanoTime()));
        ...
        int maxCandidates = minCandidates + (int) Math.log(Binom.combinations(nodesToAddEdge_withoutDuplicates.size(), 2));
        int numCandidates = 0;
        
        int i = 0;
        while (i < nodesToAddEdge_withoutDuplicates.size() && numCandidates <= maxCandidates) {
            int vi = nodesToAddEdge_withoutDuplicates.get(i);

            int j = i + 1;
            while (j < nodesToAddEdge_withoutDuplicates.size() && numCandidates <= maxCandidates) {
    ...
    protected boolean oneEdgeSwitch(Integer vi) {
        ...
        ArrayList<Integer> vks = gk.getEdges(vi);
        // randomize the list
        Collections.shuffle(vks, new Random(System.nanoTime()));
         ...
         */
        ArrayList<Integer> vjs = new ArrayList<Integer>();
        vjs.addAll(new TreeSet<Integer>(nodesToAddEdge));
        // randomize the list
        Collections.shuffle(vjs, new Random(System.nanoTime()));
        logger.debug(String.format("oneEdgeSwitch: vjs=%s from %s", vjs, nodesToAddEdge));

        int maxCandidates = minCandidates + (int) Math.log(vjs.size() * vks.size());
        int numCandidates = 0;
        
        int j = 0;
        while(j < vjs.size() && numCandidates <= maxCandidates) {
            int vj = vjs.get(j);
            ...
            int k = 0;
            while(k < vks.size() && numCandidates <= maxCandidates) {
                int vk = vks.get(k);
